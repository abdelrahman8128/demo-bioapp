...
class ScoresState extends State<ScoresView> {
  ...
  onTapGetScores(BuildContext context, bool isDaily) {
    ...
    if (isDaily) {
      SahhaFlutter.getScores(types: [SahhaScoreType.activity, SahhaScoreType.sleep, SahhaScoreType.wellbeing,SahhaScoreType.mental_wellbeing,SahhaScoreType.readiness])
          .then((value) => {showAlertDialog(context, value),print(value)})
          .catchError((error, stackTrace) => {
            debugPrint(error.toString()),
